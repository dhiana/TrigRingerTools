 ...
 * @file lms-train.cxx
 *
 * Builds an unbiased LMS discriminator from scratch, based on a database
 * (number of features and classes to separate) and train the network to
 * recognize the patterns in that database.
 */
...
int main (int argc, char** argv)
{
  ...
  testdb.merge_target(true, -1, +1, test_target);
  ...
  try {
    ...
    if (traindb.size() == 2) comment << " and SP = " << sp_test;
    comment << ".";
