...
typedef struct param_t {
  ...
  std::vector<std::string> rclasses; ///< ROOT classes
  std::vector<std::string> traindb; ///< Train DB
  std::vector<std::string> testdb; ///< Test DB

  bool xml;           ///< read from root files or XML files?
...
{
  if (par.xml) {
    if (par.traindb.size() != 1) {
      RINGER_DEBUG1("One train database must be specified for XML input!");
      throw RINGER_EXCEPTION("Multiple or zero train database files for XML reading");
    }
    if (par.traindb.size() != 1) {
      RINGER_DEBUG1("One test database must be specified for XML input!");
      throw RINGER_EXCEPTION("Multiple or zero test database files for XML reading.");
    }
    if (!sys::exists(par.traindb[0])) {
      RINGER_DEBUG1("Train Database file " << par.traindb[0] << " doesn't exist.");
      throw RINGER_EXCEPTION("Train Database file doesn't exist");
    }
    if (!sys::exists(par.testdb[0])) {
      RINGER_DEBUG1("Test Database file " << par.testdb[0] << " doesn't exist.");
      throw RINGER_EXCEPTION("Test Database file doesn't exist");
    }
  } else {
    if (par.rclasses.size() != par.traindb.size()) {
      RINGER_DEBUG1("Number of train classes must equal number of train databases! Throwing...");
      throw RINGER_EXCEPTION("Train Database information mismatch.");
    }
    if (par.rclasses.size() != par.testdb.size()) {
      RINGER_DEBUG1("Number of test classes must equal number of test databases! Throwing...");
      throw RINGER_EXCEPTION("Test Database information mismatch.");
    }
    for (std::vector<std::string>::const_iterator i = par.traindb.begin(); i != par.traindb.end(); i++) {
      if (!sys::exists(*i)) {
        RINGER_DEBUG1("Train database file " << *i << " doesn't exist!");
	throw RINGER_EXCEPTION("Train database file doesn't exist");
      }
    }
    for (std::vector<std::string>::const_iterator i = par.testdb.begin(); i != par.testdb.end(); i++) {
      if (!sys::exists(*i)) {
        RINGER_DEBUG1("Test database file " << *i << " doesn't exist!");
	throw RINGER_EXCEPTION("Test database file doesn't exist");
      }
    }
  }
...
int main (int argc, char** argv)
{
  ...
  param_t par = { std::vector<std::string>(0), std::vector<std::string>(0),
  ...
  // Only 4 letters available now: q, v, h, f
  opt_parser.add_option
    ("rclasses", 'a', par.rclasses,
     "Name of the classes to use with ROOT files.");
  opt_parser.add_option
    ("traindb", 'd', par.traindb,
     "location of the database(s) to use for training: for ROOT files specify one per class, separated by space (escaped by quotes if necessary)");
  opt_parser.add_option
    ("testdb", 'u', par.testdb,
     "location of the database(s) to use for testing: for ROOT files specify one per class, separated by spaces (escaped by quotes if necessary)");
  opt_parser.add_option
  ...
  opt_parser.add_option
    ("input-xml", 'x', par.xml,
     "should use XML files as input database");
  ...
  data::Database<data::RoIPatternSet> *traindb = 0, *testdb = 0;
  if (par.xml) {
    RINGER_DEBUG1("Using input from XML file.");
    traindb = new data::DatabaseXml<data::RoIPatternSet>(par.traindb[0], reporter);
    testdb = new data::DatabaseXml<data::RoIPatternSet>(par.testdb[0], reporter);
  } else {
    RINGER_DEBUG1("Using input from ROOT file.");
    std::map<const std::string, const data::RootClassInfo> rootTrain;
    std::map<const std::string, const data::RootClassInfo> rootTest;
    for (unsigned int i = 0; i < par.rclasses.size(); i++) {
      RINGER_DEBUG3("Inserting train file " << par.traindb[i] << " on class " << par.rclasses[i]);
      RINGER_DEBUG3("Inserting test file " << par.testdb[i] << " on class " << par.rclasses[i]);
      rootTrain.insert(make_pair(par.rclasses[i], data::RootClassInfo(par.traindb[i], "CollectionTree")));
      rootTest.insert(make_pair(par.rclasses[i], data::RootClassInfo(par.testdb[i], "CollectionTree")));
    }
    traindb = new data::DatabaseRoot<data::RoIPatternSet>(rootTrain, reporter);
    testdb = new data::DatabaseRoot<data::RoIPatternSet>(rootTest, reporter);
    RINGER_DEBUG1("TrainDB pattern size: " << traindb->pattern_size());
  }
  ...
  try {
    ...
    RINGER_DEBUG1("Original train database saved.");

    traindb->normalise();
    //calculate the normalization factor based on the train set, but only after
    //size normalization
    RINGER_DEBUG1("Train DB Normalization.");
	...
	test_energies[i] = add(test.pattern(i));
      ...
      comment << "Outputs from training session based on ";
      for (unsigned int i = 0; i < par.traindb.size(); i++) {
        comment << par.traindb.at(0) << ", ";
      }
      comment << ". Train set MSE = " << mse_train;
    ...
    RINGER_DEBUG3("Exception thrown. Deleting train and test databases.");
    if (traindb)
      delete traindb;
    if (testdb)
      delete testdb;
